...
| 指モード | MediaPipe GestureRecognizer で `Pointing_Up` を検知、人差し指先端座標で線を描画。手全体を映す都合上カメラの下半分は使えないため、カメラ映像の上部（`TOP_RATIO`）のみを画面全体の高さへマッピングする |
...
- ✅ 指トラッキングのカメラ上半分→画面全体マッピング（`handInput.js`）
  - 手全体を映す都合上カメラの下半分は使えないため、`TOP_RATIO`で指定した上部だけを画面の縦方向全体へ引き伸ばす
  - 線・餌の座標（実サイズ）は指先(landmark 8)基準、手のスケルトン描画も同じシフト量を共有し表示位置がズレないようにしている
